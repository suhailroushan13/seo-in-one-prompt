...
export function buildPrompt(s: FormState): string {
  ...
  const desc = v(s.projectDesc, "Not specified");
  const stack = s.techStack === "custom" ? v(s.customStack, "") : s.techStack;
  const primaryKwVal = v(s.primaryKw, "Not specified");
  const secondaryKwVal = v(s.secondaryKw, "Not specified");
  const audienceVal = v(s.audience, "Not specified");
  const competitorsVal = v(s.competitors, "Not specified");
  const pageTitleVal =
  ...
  const slug =
    primaryKwVal !== "Not specified"
      ? primaryKwVal.toLowerCase().replace(/\s+/g, "-")
      : "main-topic";
...
- Analyze what competitors${competitorsVal !== "Not specified" ? ` (${competitorsVal})` : ""} are doing right and outperform them.
